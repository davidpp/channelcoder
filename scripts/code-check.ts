...
async function getFilesToCheck(): Promise<{ files: string[]; mode: string; base?: string }> {
  ...
  if (values.base) {
    try {
      ...
      console.error(
        `❌ Error: Could not compare against '${values.base}'. Branch/commit may not exist.`
      );
      process.exit(1);
    }
  }
...
function parseAndFilterTSErrors(
  tsOutput: string,
  filesToShow: string[]
): { filteredErrors: string[]; totalErrors: number; filteredCount: number } {
  if (!tsOutput.trim()) {
...
async function runChecks() {
  ...
  const results = {
    ...
    files: files.length,
    biome: { success: false } as CheckResult,
    typescript: { success: false } as CheckResult,
    overall: false,
  };
