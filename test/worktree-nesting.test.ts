...
describe('Worktree nesting prevention', () => {
  ...
  test('creates sibling worktrees when run from main repo', async () => {
    const wtA = await utils.create('branch-a', { cwd: testRepoPath });
    expect(wtA.path).toBe(worktreeAPath);
    expect(existsSync(wtA.path)).toBe(true);

    const wtB = await utils.create('branch-b', { cwd: testRepoPath });
  ...
  test('creates sibling worktrees when run from within a worktree', async () => {
    ...
    const wtB = await utils.create('branch-b', { cwd: wtA.path });
  ...
  test('lists all worktrees from any location', async () => {
    // Create worktrees
    await utils.create('branch-a', { cwd: testRepoPath });
    const wtB = await utils.create('branch-b', { cwd: testRepoPath });

    // List from main repo
    const fromMain = await utils.list({ cwd: testRepoPath });
  ...
  test('detects if in a worktree correctly', async () => {
    // Test from main repo
    const inMainRepo = await utils.isInWorktree(testRepoPath);
    expect(inMainRepo).toBe(false);

    // Create a worktree
    const wt = await utils.create('branch-a', { cwd: testRepoPath });

    // Test from worktree
    const inWorktree = await utils.isInWorktree(wt.path);
    expect(inWorktree).toBe(true);
  });

  test('finds main repository from any location', async () => {
    // From main repo
    const fromMain = await utils.findMainRepository(testRepoPath);
    ...
    // From worktree
    const fromWorktree = await utils.findMainRepository(wt.path);
