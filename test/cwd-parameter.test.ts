...
describe('CWD parameter support', () => {
  ...
  test('worktreeUtils functions work with different cwd values', async () => {
    ...
    const list2 = await worktreeUtils.list({ cwd: repo2Path });
    ...
    expect(await worktreeUtils.exists('feature-1', { cwd: repo1Path })).toBe(true);
    expect(await worktreeUtils.exists('feature-1', { cwd: repo2Path })).toBe(false);
    expect(await worktreeUtils.exists('feature-2', { cwd: repo1Path })).toBe(false);
    expect(await worktreeUtils.exists('feature-2', { cwd: repo2Path })).toBe(true);

    // Find worktrees in correct repos
    const found1 = await worktreeUtils.find('feature-1', { cwd: repo1Path });
    expect(found1).not.toBeNull();
    expect(found1?.path).toBe(worktree1Path);

    const notFound1 = await worktreeUtils.find('feature-1', { cwd: repo2Path });
    expect(notFound1).toBeNull();

    // Remove worktrees with cwd
    await worktreeUtils.remove('feature-1', { cwd: repo1Path });
    expect(existsSync(worktree1Path)).toBe(false);

    await worktreeUtils.remove('feature-2', { cwd: repo2Path });
  ...
  test('worktree detection utilities work with cwd', async () => {
    // Test isInWorktree
    const inMainRepo1 = await worktreeUtils.isInWorktree(repo1Path);
    expect(inMainRepo1).toBe(false);

    const inMainRepo2 = await worktreeUtils.isInWorktree(repo2Path);
    ...
    const inWorktree1 = await worktreeUtils.isInWorktree(wt1.path);
    expect(inWorktree1).toBe(true);

    const inWorktree2 = await worktreeUtils.isInWorktree(wt2.path);
    expect(inWorktree2).toBe(true);

    // Test findMainRepository
    const mainFromRepo1 = await worktreeUtils.findMainRepository(repo1Path);
    expect(mainFromRepo1).toBe(repo1Path);

    const mainFromRepo2 = await worktreeUtils.findMainRepository(repo2Path);
    expect(mainFromRepo2).toBe(repo2Path);

    const mainFromWorktree1 = await worktreeUtils.findMainRepository(wt1.path);
    expect(mainFromWorktree1).toBe(repo1Path);

    const mainFromWorktree2 = await worktreeUtils.findMainRepository(wt2.path);
  ...
  test('current() function works with cwd parameter', async () => {
    ...
    const currentFromMain1 = await worktreeUtils.current({ cwd: repo1Path });
    expect(currentFromMain1).not.toBeNull();
    expect(currentFromMain1?.branch).toBe('main');
    expect(currentFromMain1?.path).toBe(repo1Path);

    const currentFromMain2 = await worktreeUtils.current({ cwd: repo2Path });
    ...
    const currentFromWt1 = await worktreeUtils.current({ cwd: wt1.path });
    expect(currentFromWt1).not.toBeNull();
    expect(currentFromWt1?.branch).toBe('feature-1');

    const currentFromWt2 = await worktreeUtils.current({ cwd: wt2.path });
  ...
  test('cleanup() function works with cwd parameter', async () => {
    ...
    await worktreeUtils.create('feature-test', { cwd: repo1Path, base: 'main' });
    await worktreeUtils.create('experiment-test', { cwd: repo2Path, base: 'main' });

    // Test cleanup dry run for repo1
    const toCleanup1 = await worktreeUtils.cleanup({ dryRun: true, cwd: repo1Path });
    ...
    const toCleanup2 = await worktreeUtils.cleanup({ dryRun: true, cwd: repo2Path });
    ...
    const cleaned1 = await worktreeUtils.cleanup({ cwd: repo1Path });
    expect(cleaned1.length).toBeGreaterThan(0);

    // Verify repo2 worktrees still exist
    const list2After = await worktreeUtils.list({ cwd: repo2Path });
