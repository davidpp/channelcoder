...
import type { SessionStorage, SessionState } from '../src/session.js';
...
describe('Session E2E Tests', () => {
  ...
  test('session list functionality', async () => {
    ...
    const session1State: SessionState = {
      ...
      currentSessionId: 's1',
      messages: [
        { role: 'user', content: 'Test 1', timestamp: new Date(), sessionId: 's1' },
      ],
      metadata: {
...
Continue our discussion about the bug.`);
  ...
  test('session clear functionality', async () => {
    ...
    const sessionState: SessionState = {
      ...
      currentSessionId: 's2',
      messages: [
        { role: 'user', content: 'Test', timestamp: new Date(), sessionId: 's1' },
      ],
      metadata: {
        name: 'test-clear',
        created: new Date(),
